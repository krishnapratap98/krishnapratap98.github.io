...
import Krishna from "../images/mailgif/Resume_pic.jpg";
...
export const Banner = () => {
    ...
    const onButtonClick = () => {
        fetch("Krishna_Pratap Singh_CV.pdf").then((response) => {
          response.blob().then((blob) => {
            const fileURL = window.URL.createObjectURL(blob);
            let alink = document.createElement("a");
            alink.href = fileURL;
            alink.download = "Krishna_Pratap Singh_CV.pdf";
            alink.click();
          });
        });
      };
                        ...
                        <TrackVisibility>
                            {({ isVisible }) =>
                                <div className={isVisible ? "animate__animated animate__fadeIn" : ""}>
                                    ...
                                    <h1>Hi! I'm <i style={{color:"rgb(17,139,150)"}}>Krishna Pratap Singh</i>  <span className="txt-rotate" dataperiod="1000" data-rotate='[ "Aspiring Full Stack Web Developer",  "Aspiring Full Stack Web Developer", "Aspiring Full Stack Web Developer",]'><span className="wrap">{text}</span></span></h1>
            ...
            <TrackVisibility>
              {({ isVisible }) =>
                <div className={isVisible ? "" : ""}>
                  <img className="myimage" src={Krishna} alt="Header Img"/>
                </div>}
            </TrackVisibility>
          </Col>
